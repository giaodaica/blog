<div class="row variant-item gx-2 gy-3 align-items-end border-bottom pb-3 mb-4">
    ...
    <div class="col-lg-2">
        ...
        <div class="d-flex align-items-center">
            <select class="form-select me-2 color-select @error("variants.$index.color_id") is-invalid @enderror"
                name="variants[{{ $index }}][color_id]" data-color-preview="color-preview-{{ $index }}" required>
            ...
            <div id="color-preview-{{ $index }}" class="border rounded"
                ...
                @php
                    ...
                    echo $selectedColor ? $selectedColor->color_code : '#fff';
                @endphp;">
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="input-group">
            <input type="file" class="form-control variant-image-input @error("variants.$index.variant_image") is-invalid @enderror"
    ...
    @if ($errors->hasAny([
        ...
        <div class="col-12 mt-2">
            <div class="alert alert-danger p-2 border rounded-3 bg-light border-danger-subtle">
                <div class="fw-semibold mb-1 text-danger-emphasis">⚠️ Biến thể này có lỗi:</div>
                <ul class="mb-0 ps-3 small">
                    @foreach ($errors->get("variants.$index.size_id") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.color_id") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.import_price") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.listed_price") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.sale_price") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.stock") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.variant_image") as $msg) <li>{{ $msg }}</li> @endforeach
                    @foreach ($errors->get("variants.$index.temp_variant_image_url") as $msg) <li>{{ $msg }}</li> @endforeach
                </ul>
            </div>
        </div>
    @endif

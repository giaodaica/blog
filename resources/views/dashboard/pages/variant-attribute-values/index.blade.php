                            ...
                            <div class="card-header border-0">
                                <div class="row g-4">
                                    <div class="col-sm-auto">
                                        <div>
                                            <a href="{{ route('variant-attributes-values.create') }}"
                                                class="btn btn-success" id="addproduct-btn"><i
                                                    class="ri-add-line align-bottom me-1"></i> Thêm giá trị
                                                thuộc tính</a>
                                        </div>
                                    </div>
                                    <div class="col-sm">
                                ...
                                <table class="table table-bordered table-striped align-middle text-center">
                                    ...
                                    <tbody>
                                        @foreach ($values as $index => $value)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $value->variant_attribute->name ?? 'Chưa có' }}</td>
                                                <td>{{ $value->value }}</td>
                                                <td>
                                                    <a href="{{ route('variant-attributes-values.edit', $value->id) }}"
                                                        class="btn btn-sm btn-warning me-1">Sửa</a>

                                                    <form
                                                        action="{{ route('variant-attributes-values.destroy', $value->id) }}"
                                                        method="POST" style="display:inline-block;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger"
                                                            onclick="return confirm('Bạn có chắc muốn xóa giá trị thuộc tính này?');">Xóa</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
